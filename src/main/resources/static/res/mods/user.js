...
layui.define(['laypage', 'fly', 'element', 'flow','form'], function(exports){
  ...
  form.on('submit(reg)', function(data){
    ...
    fly.json('/user/register',field,function (res) {
      if(res.data === '0'){
    ...
    var tpl = [
      ...
      <li>\
        {{# if(d.rows[i].collection_time){ }}\
          <a class="post-title" href="/post/{{d.rows[i].id}}/" target="_blank">{{= d.rows[i].title}}</a>\
        ...
        {{# } else { }}\
          ...
          {{# if(d.rows[i].accept >= 0){ }}\
            <span class="post-status post-status-ok">已结</span>\
          {{# } else { }}\
            <span class="post-status">未结</span>\
          {{# } }}\
          {{# if(d.rows[i].status == -1){ }}\
            <span class="post-status">审核中</span>\
          {{# } }}\
          <a class="post-title" href="/post/{{d.rows[i].id}}/" target="_blank">{{= d.rows[i].title}}</a>\
          <i class="layui-hide-xs">{{ layui.util.timeAgo(d.rows[i].time, 1) }}</i>\
          {{# if(d.rows[i].accept == -1){ }}\
          <a class="mine-edit layui-hide-xs" href="/post/edit/{{d.rows[i].id}}" target="_blank">编辑</a>\
          {{# } }}\
      ...
      if(gather.minelog[type + '-page-' + curr]){
        ...
        if(type === 'collection'){
          ...
          fly.json('/api/'+ type +'/', {
            ...
            view(res);
            gather.minelog['mine-post-page-' + curr] = res;
            now || laypage.render({
              elem: 'LAY_page'
              ,count: res.count
              ,curr: curr
